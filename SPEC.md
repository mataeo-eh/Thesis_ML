...
| `train.*` (lr / betas / weight_decay / lr_floor / grad_clip / accum / precision) | 3e-4 / (0.9,0.95) / 0.1 / 0.01×peak / 1.0 / 5 / bf16 | Five microbatches per optimizer update; step planning uses `ceil(batches/5)` so scheduler/EMA horizons are expressed in optimizer steps. |
| `train.lr_schedule` | `wsd` | Config-selectable `wsd`, `cosine`, or `linear`. WSD uses 10% linear warmup, 70% at peak, and 20% linear decay from `3e-4` to `3e-6`. Historical profiles pin their prior schedule explicitly. |
