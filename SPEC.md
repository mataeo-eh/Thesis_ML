...
- Canvas corruption samples one global `t ~ Uniform(0,1)` per example. In default uniform mode, each eligible canvas position independently retains its clean token with probability `1-t`; otherwise it is replaced by a uniformly sampled allowed noise state. A corruption draw may coincidentally equal the clean token. Input-side fog remains independent and never applies to the output canvas.
...
- **Uniform-mode pretraining objective:** unweighted clean-state `x0` cross-entropy over every valid target-canvas position except clamped `[BOS]`, including semantic `[PAD]` targets. It does not use the Bernoulli corruption branch as a score mask and does not apply inverse-`t` weighting. Clamped `[BOS]` and batch-shape padding are excluded. Pretraining never reads `config.loss.class_loss_weights`; that section must be absent from a pretraining config.
...
| `train.*` (lr / betas / weight_decay / warmup / lr_floor / grad_clip / accum / precision) | 3e-4 / (0.9,0.95) / 0.1 / 2000 / 0.1×peak / 1.0 / as-needed / bf16 | Decay to `lr_floor` over the derived step horizon; accumulation derives from the target effective batch size. |
| `train.lr_decay_shape` | `cosine` | Post-warmup decay curve, `cosine` or `linear`. `linear` leaves the peak immediately on a constant, shallower-than-cosine slope; pair it with a lower `lr_floor_ratio` when late-training loss hovers around a minimum rather than settling. Profile-owned: the overfit-V2 and ablation profiles use `linear` / `0.03`. |
