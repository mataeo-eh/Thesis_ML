...
- The OUTPUT vocabulary contains NO coordinates, frame numbers, or absolute times. High-fidelity map position, unit stats, and absolute clock are NEVER tokens — they are input-side contextual encodings. The tokenizer carries their raw VALUES on each token record (read from the extractor parquet); the MODEL turns those values into additive embeddings in a later prompt. The tokenizer itself computes no embeddings and no positional encodings.
...
2. **Serialization** (`./src/<pkg>/serialize.py` or similar):
   ...
   - Produce ONE sequence of token records. Each record carries its token ID plus the raw input-only field values for that entity — precise map position, stats, allegiance — and its timestep's absolute clock, read directly from the extractor parquet. This is the tokenizer producing a single object, not tokens plus a side-channel. None of these raw fields enter the vocabulary; the model embeds them later. Document this record structure clearly — it is the contract prompts 003 and 004 depend on.
...
- Input-only fields (map position, stats, clock, self/enemy allegiance) are carried as raw values on each token record for the later input-representation prompt, NOT in the vocabulary, and the tokenizer computes no embeddings or positional encodings.
