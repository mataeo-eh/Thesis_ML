...
- `local_overfit_v2.yaml` owns the current learnability probe: isolated checkpoint/log/cache paths, the uniform process, and the RUN LENGTH AND LR DECAY SHAPE for itself and for every `ablation_*.yaml` arm that extends it — `epochs: 100` (34 steps/epoch = 3400 steps) with `lr_decay_shape: linear` and `lr_floor_ratio: 0.03`, overriding the 150 / cosine / 0.1 it would otherwise inherit. Everything about the data subset and model scale still comes from `local_overfit.yaml` unchanged. It carries no per-class loss weighting (that knob is fine-tuning-only). It is also the designated **ablation control surface** for the prompt-009 `model:` toggles (see Local Contracts).
