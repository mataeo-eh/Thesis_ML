...
from thesis_diffusion.train.loop import TrainStepLog, TrainingLoop
...
def _find_project_root() -> Path:
    ...
    The root is identified by the presence of the canonical ``config/default.yaml``
    that every run profile deep-merges over. We search for that marker instead of
    counting parent directories, because the number of directories between this
    module and the repository root depends on the package layout: it was 3 under
    the old single-package ``src/thesis_ml/`` tree and is 5 under the current
    ``packages/thesis-diffusion/src/thesis_diffusion/`` workspace tree. A marker
    search survives the next layout change; a hardcoded index does not.

    Returns:
        Path to the repository root directory.

    Raises:
        RuntimeError: if no ancestor directory contains ``config/default.yaml``.

    Called by: module-level ``PROJECT_ROOT`` initialization below.
    """
    ...
    raise RuntimeError(
        ...
        f"{Path(__file__).resolve()} contains config/default.yaml."
    )
