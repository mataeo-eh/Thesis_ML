...
- Own the dense bidirectional uniform-diffusion network, absorbing ablation compatibility, learned input embeddings, expected-embedding self-conditioning, and clean-state canvas loss.
...
- The EOS-terminated input region is clamped: never noised, never receives loss. Canvas position-0 BOS is attended but also clamped and unscored; loss is computed on the remaining canvas positions.
...
- Conditioning in this arm is clamping only: no encoder-decoder split, no cross-attention, no copy mechanism, no classification head, no set/pooling module. Introducing any of these is an architecture change to a measured arm, so confirm with the owner before writing code rather than adding one as a performance fix. This constrains Arm A only; Arm B's architecture is contracted separately in `thesis_ar/AGENTS.md`.
...
Three `model:` config booleans exist ONLY to run the prompt-009 representational ablation (originally motivated by outcome behavior at the old canvas index 0; outcome is now index 1). **All three default to `false`, and `false` is the baseline.** They are not staged features, not partially-rolled-out defaults, and not something to enable "since it is there". Enabling one is an experiment the owner runs; promotion to default is the owner's decision on measured evidence, never an implementing agent's.

- `model.frozen_input_kv` — two-pass forward; per-layer input K/V captured post-RoPE/post-QK-norm and reused. **DEFAULT `true`** (owner promotion 2026-08-09 on arm-01 evidence: no meaningful loss difference, materially faster inference). Adds ZERO parameters. Real semantic cost: the input stops attending to the canvas, so attention becomes one-directional across the seam. **Both passes must be handed explicit absolute RoPE position ids over the concatenation** — `RotaryEmbedding` falls back to `arange(seq_len)` when given none, and on pass 2 `seq_len` is the CANVAS length, which silently restarts the canvas at position 0 and makes the canvas-to-input offset depend on the batch's left padding. That was a live bug until 2026-08-09; `BidirectionalTransformer.forward` now builds the ids explicitly and `tests/test_model.py::test_frozen_input_kv_canvas_positions_continue_the_input_and_survive_left_padding` pins it.
- `model.segment_embeddings` — learned `nn.Embedding(2, d_model)`, 0 = input / 1 = canvas, added to the FINAL per-region embedding so it lands after the joint residual and self-conditioning post-norm rather than being renormalized away. Zero-initialized, and NOT constructed at all when off so no `state_dict` key appears and matching toggle-off v2 checkpoints load strictly. `reset_segment_embeddings()` must stay called after `_init_weights`, which would otherwise re-init it at `std=0.02`.
- `model.per_segment_positions` — per-example RoPE position ids; `cos`/`sin` gain a batch dimension `[B, S, D]`. Adds ZERO parameters. It does NOT add an absolute landmark: it re-anchors the input↔canvas relation from terminal input EOS to the input's START, trading away the fixed `-1` EOS↔BOS seam adjacency. Read `diagnostics/009-rare-class-position-blindness.md` §7.2/§7.2a before reasoning about it.

Hard invariants when touching any of them:

- With all three off, ablation-specific code adds nothing to the current vocabulary-v2 baseline, including the `[S, D]` RoPE broadcast and shared `torch.arange` positions.
- `architecture_identity` is `ARCHITECTURE_ID + toggle_fingerprint(model_config)` and equals exactly `"dense-multinomial-SC2-v2"` with all toggles off. Vocabulary-v1 checkpoints must fail closed. Never bump `ARCHITECTURE_ID` merely for a toggle; fingerprints prevent silent cross-arm loads for zero-parameter toggles.
...
- Uniform-mode loss predicts clean `x0` at every valid canvas position except clamped BOS, without inverse-time or corruption-mask weighting. The absorbing ablation retains its masked inverse-time objective. Per-token-class loss logging remains mandatory.
