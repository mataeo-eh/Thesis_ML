...
from thesis_diffusion.eval.finetune_report import build_and_write_finetune_report
from thesis_diffusion.model.model import SC2StrategyDiffusionModel
from thesis_diffusion.pipeline.finished_export import export_finished_model
from thesis_shared.pipeline.storage import StorageResolver
from thesis_diffusion.train.loop import TrainingLoop, optimizer_steps_per_epoch
from thesis_diffusion.train.train import SMOKE_VOCAB_SIZE, make_synthetic_examples
...
def run_training_pipeline(
    ...
    *,
    ...
    if not use_smoke and not _dataset_available(config, resolver) and config.pipeline.auto_acquire:
        from thesis_shared.pipeline.acquire_data import run_acquisition
