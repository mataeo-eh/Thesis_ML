# SPEC-legacy.md — RETIRED planning artifact (historical reference only)

> **This document is NOT binding and is NOT a source of truth.**
>
> It is the original planning specification from the project's first design phase,
> written when the repository targeted a single uniform discrete-diffusion model.
> It is retained for visibility into how the project was initially scoped, and for
> nothing else.
>
> **Retired 2026-09-20.** The project now runs a two-arm controlled comparison
> (discrete diffusion in PyTorch vs. an autoregressive baseline in TensorFlow),
> which this document predates and in places directly contradicts — §14 bans
> autoregressive generation outright, which is now half the experiment.
>
> **Do not read this to learn current architecture, and do not cite it in a
> contract, plan, prompt, or code comment.** Binding project rules live in the
> `AGENTS.md` DOX contract tree and `CLAUDE.md`. Implemented model state lives in
> `Model_Architecture/MODEL_ARCHITECTURE.md`. Where this file disagrees with
> either, this file is simply wrong and the other wins.
>
> Nothing in §11 (provisional parameters), §12 (open questions), §14 (banned
> list), §14a (gated items), or §14b (toggle discipline) carries any remaining
> force. The `model.frozen_input_kv`, `model.segment_embeddings`, and
> `model.per_segment_positions` config fields still exist in code with their
> current defaults; their behavior is owned by `config/default.yaml` and the
> diffusion arm's contract, not by this document.
...
- **Paired stochastic training:** production examples derive fog, canvas corruption, and self-conditioning draws from the configured base seed, epoch, and stable manifest index. The per-example seed is independent of DataLoader worker assignment and microbatch boundaries, so capacity-ablation arms with different batch sizes see the same shuffled example order and the same stochastic version of each example. The fallback stateful training generator is checkpointed for direct/synthetic callers; mid-epoch resume therefore continues rather than replaying its epoch-start draw stream.
...
- **EMA (SOTA diffusion practice):** maintain an exponential-moving-average copy of the weights during training; use EMA weights for validation, the final checkpoint, sampling, and evaluation. EMA is standard for diffusion training and one of the practices distinguishing it from AR pretraining. The decay is not a fixed constant: it is DERIVED from the configured optimizer-step schedule horizon. Ordinarily that is the run's own horizon, so the window completes inside the run. `train.ema_horizon_ratio` (default `0.1`) sets the window as a fraction of scheduled steps and `train.ema_decay` (`0.9999`, a ~10,000-step window) is the ceiling. A short learning-curve ablation may explicitly retain a longer reference horizon with `train.schedule_horizon_epochs`; this keeps LR and EMA on the reference run's opening trajectory even though the ablation intentionally stops before either schedule completes.
...
| `train.schedule_horizon_epochs` | `0` | `0` fits LR/EMA to the actual run. A positive value deliberately retains that many epochs as the optimizer-step schedule horizon while `train.epochs` remains the stop limit; used by the three-epoch capacity suite to preserve the 50-epoch V3 opening schedule. |
...
**Model-sizing note.** All `model.*` values are config; size changes require no code changes. V3 is the current 29.3M full-corpus shape. The capacity suite spans 4.75M, 15.08M, the 29.32M V3 baseline, a 30.21M 18-layer depth control, 59.36M, and 121.47M while retaining dense GeGLU, vanilla MHA, 64-dimensional heads, QK-norm, RoPE, sandwich RMSNorm, and every other settled mechanism. Historical ~11M profiles remain reproducible through explicit overrides.
