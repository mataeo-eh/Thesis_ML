...
- Optimizer/schedule/checkpoint fields (`lr`, `betas`, `weight_decay`, schedule choice, fixed warmup steps, WSD decay ratio, `lr_floor`, `grad_clip`, accumulation, precision, epochs, early stopping, cadence, and family subdirectories) are config-owned. Derived step horizons divide dataloader batches by fixed accumulation.
