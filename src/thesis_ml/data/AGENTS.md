...
- In both modes, each input timestep serializes self records, fog-filtered enemy records, and exactly one delimiter; one `[EOS]` follows the last delimiter and counts against every window's input budget. Fog samples one rate per served example from `fog.rate_distribution` (uniform 0.0-0.8 by default), then independently omits each enemy content record, including upgrades, from the clamped input. Self records, delimiters, and EOS remain; the clean enemy sequence still owns target construction. Persisted artifacts and manifests stay clean — never bake fog into them.
