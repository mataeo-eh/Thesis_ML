...
- `embedding.py` owns allowlisted input features and joint token-feature conditioning (`InputFeatures`, `build_input_features`, `InputContextEmbedding`): standardized map position/unit stats, numeric allegiance, and feature-masked residual mixing.
...
- Joint static conditioning is input-only; canvas tokens never receive map position, unit stats, or allegiance. Continuous features are standardized by the persisted training-split artifact, concatenated with numeric allegiance, projected by `Linear(F,32) -> ReLU -> Linear(32,32) -> ReLU`, then mixed jointly with token embeddings by `Linear(d_model+32,d_model) -> GELU -> Linear(d_model,d_model)`.
