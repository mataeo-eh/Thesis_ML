        ...
        [
            {
                ...
                "p2_bot_marine_001_pos_(X,Y,Z)": "(1.0, 1.0, 0.0)",
                "p2_bot_marine_001_health": 45.0,
                "p2_bot_marine_002_pos_(X,Y,Z)": None,
                "p2_bot_marine_002_health": None,
                "p2_bot_barracks_001_pos_(X,Y,Z)": None,
            ...
            },
            {
                ...
                "p2_bot_marine_002_pos_(X,Y,Z)": None,
                "p2_bot_marine_002_health": None,
                "p2_bot_barracks_001_pos_(X,Y,Z)": "(2.0, 2.0, 0.0)",
            ...
            },
            {
                ...
                "p2_bot_marine_001_pos_(X,Y,Z)": "(1.0, 1.0, 0.0)",
                "p2_bot_marine_001_health": 45.0,
                "p2_bot_marine_002_pos_(X,Y,Z)": "(3.0, 3.0, 0.0)",
...
def test_boundary_truncated_timesteps_are_not_dropped() -> None:
    config = _small_config(canvas_budget=6)
    base = make_synthetic_examples(config, count=1)[0]
    # Canvas leads with [BOS], then [WIN]; the single timestep
    # (token 100) follows before the truncating [DELIMITER]/[PAD] tail.
    truncated = replace(
        base,
        target_canvas=torch.tensor([BOS_ID, WIN_ID, CONTENT_TOKEN_OFFSET, DELIMITER_ID, PAD_ID, PAD_ID], dtype=torch.long),
        class_labels=torch.tensor(
            [CLASS_CLAMPED, CLASS_WINLOSS, CLASS_ENEMY_FUTURE, CLASS_DELIMITER, CLASS_PAD, CLASS_PAD], dtype=torch.long
        ),
        terminated=False,
        truncated=True,
        canvas_metadata=[],
    )
    model = FixedCanvasModel(torch.tensor([BOS_ID, WIN_ID, CONTENT_TOKEN_OFFSET, DELIMITER_ID, PAD_ID, PAD_ID]), vocab_size=128)
